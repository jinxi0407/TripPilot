...
test('一键 Demo、天气修订和预算冲突',async({page})=>{
 ...
 await page.route('**/api/plan',async route=>{const payload=route.request().postDataJSON();await route.continue({postData:JSON.stringify({...payload,constraints:{...payload.constraints,total_budget:20000}})})})
 await page.getByRole('button',{name:/三城慢游/}).click()
 await expect(page.getByText('仍有已确认的约束冲突，需要调整')).toBeVisible()
 await expect(page.getByLabel('调整总预算')).toBeVisible()
