...
export function ActivityPanel({run,itinerary,selected}:{run:Run|null;itinerary:Itinerary|null;selected:number}){
 ...
 return <aside className="activity-panel">
  <section className="agent-card"><div className="panel-heading"><h2><Activity size={17}/>Agent Activity</h2><span className="live-dot"/></div><p className="muted">五个角色，一起把旅程想周全。</p>
   ...
   <div className="agent-footnote">{run?`${run.metrics.tool_attempts} 次工具请求 · ${Object.values(run.metrics.agent_steps).reduce((a,b)=>a+b,0)} 个规划步骤`:'可查看工具动作与安全执行摘要'}</div>
  </section>
  <section className="map-card"><div className="panel-heading"><h2><Route size={17}/>沿途一览</h2><span className="tiny-pill">{day?`DAY ${day.day}`:'路线预览'}</span></div><LiveMap day={day} enabled={run?.mode!=='fixture'}/>
   <div className="map-points">{day?.activities.map((a,i)=><div key={i}><span className="point-number">{i+1}</span><div><strong>{a.poi.name}</strong><small>{a.poi.coordinates?`${a.poi.coordinates.longitude.toFixed(4)}, ${a.poi.coordinates.latitude.toFixed(4)} · GCJ-02`:'经纬度待确认'}</small></div></div>)??<p className="muted">生成行程后，查看当天的景点与坐标。</p>}</div>
  </section>
  {itinerary&&<section className="budget-card"><div className="panel-heading"><h2><Wallet size={17}/>旅行账本</h2><span className="tiny-pill">估算</span></div><div className="budget-total">{money(total)}<span>/ {money(budget)}</span></div><div className={`budget-track ${budget!=null&&total>budget?'over':''}`}><span style={{width:`${budget?Math.min(100,total/budget*100):0}%`}}/></div>{Object.entries(itinerary.costs.categories).map(([key,value])=><p key={key}><span>{categoryText[key]}</span><strong>{money(value)}</strong></p>)}<div className="budget-note">{itinerary.costs.unknown_items.length?'有未确认费用，尚不能确认预算合规。':budget!=null?(total<=budget?`预算余量 ${money(budget-total)}`:`超出预算 ${money(total-budget)}`):'未设置预算上限'}</div></section>}
 </aside>
}
