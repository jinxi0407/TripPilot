...
def evaluate(case, state, contexts, system):
    ...
    issues = (
        [i.type for i in validation.issues if i.blocking]
        if validation
        else ["NO_ITINERARY"]
    )
    hard = {}
    ...
    legacy_replanning = replanning
    replanning = strict_replanning(state, system, runtime_fail)
    memory = None
    override = None
    ...
    if "Transport" in case.expected_agent_routes:
        transport = bool(
            ...
            and not runtime_fail
            and all(
                d.rail or d.flight for d in itinerary.days if d.city != d.origin_city
            )
            and not any(
                x in issues
                for x in [
    ...
    return {
        ...
        "legacy_replanning_success": legacy_replanning,
...
def strict_replanning(state, system, runtime_fail=False):
    """Observed confirmed conflict opportunities; uncertainty/no opportunity is N/A.

    Per-case success requires every actual blocking conflict be removed after a
    Critic-requested replan, with a subsequent Critic pass and no final blocker.
    An observed conflict stopped before attempting repair is a failure, not N/A.
    """
    if system == "baseline":
        return None
    history = state.get("validation_history", [])
    opportunities = [(n, h) for n, h in enumerate(history) if h.get("blocking")]
    if not opportunities:
        return None
    if runtime_fail or not history or history[-1].get("blocking"):
        return False
    ...
    def identity(issue):
        return (
            issue["type"],
            issue.get("day"),
            issue.get("activity_id") or issue.get("target"),
        )

    for n, h in opportunities:
        if not h.get("replanning_requested"):
            return False
        initial = {identity(i) for i in h["blocking"]}
        if not any(
            not initial.intersection(identity(i) for i in later.get("blocking", []))
            for later in history[n + 1 :]
        ):
            return False
    return True
